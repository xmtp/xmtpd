#!/usr/bin/env bash
#
# nolimit_sweep2.sh — WAF + app rate limits OFF. Find the breaking point.
#
set -euo pipefail
...
SNAP_DIR="snapshots/nolimit2_${TIMESTAMP}"
mkdir -p "${SNAP_DIR}"

# Start at 200K since we proved that's clean. Go BIG.
DAU_TIERS=(200000 400000 600000 800000 1000000 1500000 2000000 3000000 5000000)

log() {
  echo "[$(date -u +%H:%M:%S)] $*"
}

log "FULL NO-LIMIT CB user workload — FIND THE BREAKING POINT"
log "Target: ${ADDR} | Duration per tier: ${DUR}"
log "Config: WAF=10M/5min, app rate limits OFF, single Ohio node"
log "Tiers: ${DAU_TIERS[*]}"
log "Output: ${SNAP_DIR}/"
echo ""
...
cat > "${SNAP_DIR}/meta.json" << EOF
{
  ...
  "type": "full_nolimit_cb_user_workload",
  "config": "waf_10M_5min, app_rate_limits_off, single_ohio_node"
}
EOF
...
for dau in "${DAU_TIERS[@]}"; do
  ...
  log "================================================================"
  log "  ${dau_k}K DAU  |  target ${target_rps} req/s"
  log "================================================================"
...
print()
print('==============================================================================')
print('  FULL NO-LIMIT CB USER WORKLOAD (WAF 10M, app RL off, single Ohio node)')
print('==============================================================================')
print()
print('+-----------+-----------+-----------+-----------+-----------+--------+-----------+')
...
# Per-API for the last few tiers
for t in tiers[-3:]:
    dau_k = t['dau'] // 1000
    print(f'\n--- {dau_k}K DAU per-API ---')
