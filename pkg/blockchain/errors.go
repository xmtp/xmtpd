...
var protocolErrorsDictionary = map[string]string{
...
func protocolErrorFromCode(code string) (errorCode string, exists bool) {
	if err, ok := protocolErrorsDictionary[code]; ok {
		return err, true
	}
...
func tryExtractProtocolError(err error) (string, bool) {
	if err == nil {
		return "", false
	}
	...
	errorCode, exists := protocolErrorFromCode(matches[1])
