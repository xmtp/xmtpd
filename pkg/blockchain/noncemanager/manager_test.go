...
func setupTestManagers(t *testing.T) []TestManager {
	...
	managers = append(managers, TestManager{
		name:    "SQL",
		manager: sqlManager,
	})
	...
	if redisClient != nil {
		redisManager := redismanager.NewRedisBackedNonceManager(redisClient, logger, keyPrefix)
		managers = append(managers, TestManager{
			name:    "Redis",
	...
	for _, tm := range managers {
		t.Run(tm.name, func(t *testing.T) {
			...
			if tm.name == "SQL" {
				...
				redisClient, keyPrefix := redistestutils.NewRedisForTest(t)
				if redisClient == nil {
					t.Skip("Redis not available for multi-instance test")
					return
				}

				for i := 0; i < numManagers; i++ {
					managerInstances = append(managerInstances,
						redismanager.NewRedisBackedNonceManager(redisClient, logger, keyPrefix))
				}
			}
...
func TestFastForward(t *testing.T) {
	...
	for _, tm := range managers {
		t.Run(tm.name, func(t *testing.T) {
			...
			nonce, err := tm.manager.GetNonce(ctx)
			require.NoError(t, err)
			require.GreaterOrEqual(t, nonce.Nonce.Int64(), int64(1000),
				"Nonce after fast-forward should be >= 1000")
			err = nonce.Consume()
			require.NoError(t, err)

			// Verify sequential allocation from fast-forward point
			expectedNonce := int64(1000)
			if nonce.Nonce.Int64() != expectedNonce {
...
// TestReplenishContinuity verifies nonces are allocated continuously from replenish points
func TestReplenishContinuity(t *testing.T) {
	ctx := context.Background()
	managers := setupTestManagers(t)

	for _, tm := range managers {
		t.Run(tm.name, func(t *testing.T) {
			// Replenish starting from 100
			err := tm.manager.Replenish(ctx, *big.NewInt(100))
			require.NoError(t, err)

			// Allocate and consume nonces to verify continuity
			for i := int64(100); i < 110; i++ {
				nonce, err := tm.manager.GetNonce(ctx)
				require.NoError(t, err)
				require.EqualValues(t, i, nonce.Nonce.Int64(),
					"Nonces should be continuous starting from replenish point")
				err = nonce.Consume()
				require.NoError(t, err)
			}

			// Replenish from a different starting point
			err = tm.manager.Replenish(ctx, *big.NewInt(500))
			require.NoError(t, err)

			// Should continue from 500 (since we consumed 100-109)
			nonce, err := tm.manager.GetNonce(ctx)
			require.NoError(t, err)
			require.EqualValues(t, 500, nonce.Nonce.Int64())
			err = nonce.Consume()
			require.NoError(t, err)
		})
	}
