...
type PayerOptions struct {
	...
	NodeSelectorStrategy        string        `long:"node-selector-strategy"          env:"XMTPD_PAYER_NODE_SELECTOR_STRATEGY"          description:"Node selection strategy: stable (default), manual, ordered, random, closest" default:"stable"`
	NodeSelectorPreferredNodes  []uint32      `long:"node-selector-preferred-nodes"   env:"XMTPD_PAYER_NODE_SELECTOR_PREFERRED_NODES"   description:"Preferred node IDs for manual and ordered strategies (comma-separated)"`
	...
	NodeSelectorConnectTimeout  time.Duration `long:"node-selector-connect-timeout"   env:"XMTPD_PAYER_NODE_SELECTOR_CONNECT_TIMEOUT"   description:"Connection timeout for closest strategy latency measurements"          default:"2s"`
}
