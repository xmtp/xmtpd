...
| `gateway_envelopes_blobs`      | Envelope payloads (cold path, partitioned)             | `originator_node_id`, `originator_sequence_id`, `originator_envelope`                                         |
...
`gateway_envelopes_meta` and `gateway_envelopes_blobs` use two-level partitioning:
...
- **Blob table** (`gateway_envelopes_blobs`): large payloads, joined only when envelope content is needed
