...
WHERE payer_id = $2
	AND (
		$3::INTEGER = 0
		OR minutes_since_epoch > $3::INTEGER
	)
	AND (
		$4::INTEGER = 0
		OR minutes_since_epoch < $4::INTEGER
	)
...
type GetPayerInfoReportParams struct {
	GroupBy             interface{}
	PayerID             int32
	MinutesSinceEpochGt int32
	MinutesSinceEpochLt int32
}
...
func (q *Queries) GetPayerInfoReport(ctx context.Context, arg GetPayerInfoReportParams) ([]GetPayerInfoReportRow, error) {
	rows, err := q.db.QueryContext(ctx, getPayerInfoReport,
		arg.GroupBy,
		arg.PayerID,
		arg.MinutesSinceEpochGt,
		arg.MinutesSinceEpochLt,
	)
	if err != nil {
		return nil, err
	}
