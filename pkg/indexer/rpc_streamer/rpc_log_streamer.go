...
func (r *RPCLogStreamer) Start() error {
	for _, watcher := range r.watchers {
		...
		tracing.GoPanicWrap(
			r.ctx,
			&r.wg,
			fmt.Sprintf("rpcLogStreamer-watcher-%v", watcher.Contract.Address()),
...
func (r *RPCLogStreamer) watchContract(cfg *ContractConfig) {
	var (
		logger = r.logger.With(
			zap.String("contractID", cfg.ID),
			zap.String("contractAddress", cfg.Contract.Address().Hex()),
		)
		backfillFromBlockNumber, backfillFromBlockHash = cfg.Contract.GetLatestBlock()
	)
...
backfillLoop:
	for {
		...
		default:
			logger.Debug(
				"Getting next page",
				zap.Uint64("fromBlockNumber", backfillFromBlockNumber),
				zap.String("fromBlockHash", common.Hash(backfillFromBlockHash).Hex()),
			)

			response, err := r.GetNextPage(r.ctx, cfg, logger, backfillFromBlockNumber, backfillFromBlockHash)
			if err != nil {
				switch err {
			...
			if len(response.Logs) == 0 {
				...
				if err := cfg.Contract.UpdateLatestBlock(r.ctx, backfillFromBlockNumber, backfillFromBlockHash); err != nil {
					logger.Error("failed to update latest block", zap.Error(err))
				}
	...
	metrics.EmitIndexerCurrentBlockLag(cfg.Contract.Address().Hex(), 0)
	...
	for {
		...
		case log, open := <-innerSubCh:
			...
			metrics.EmitIndexerCurrentBlock(cfg.Contract.Address().Hex(), log.BlockNumber)
			metrics.EmitIndexerNumLogsFound(cfg.Contract.Address().Hex(), 1)
			metrics.EmitIndexerMaxBlock(cfg.Contract.Address().Hex(), log.BlockNumber)
...
func (r *RPCLogStreamer) GetNextPage(
	...
	contractAddress := cfg.Contract.Address().Hex()
...
func (r *RPCLogStreamer) buildSubscriptionChannel(
	...
	tracing.GoPanicWrap(
		r.ctx,
		&r.wg,
		fmt.Sprintf("rpcLogStreamer-watcher-subscription-%v", cfg.Contract.Address().Hex()),
		func(ctx context.Context) {
...
func (r *RPCLogStreamer) validateWatcher(cfg *ContractConfig) error {
	...
	if err != nil {
		return fmt.Errorf("failed to validate watcher %s: %w", cfg.Contract.Address().Hex(), err)
	}
...
func buildBaseFilterQuery(cfg ContractConfig) ethereum.FilterQuery {
	return ethereum.FilterQuery{
		Addresses: []common.Address{cfg.Contract.Address()},
		Topics:    [][]common.Hash{cfg.Contract.Topics()},
	}
}
