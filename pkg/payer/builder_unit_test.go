...
import (
	...
	"github.com/xmtp/xmtpd/pkg/testutils/network"
	"go.uber.org/zap"
)
...
func createMinimalTestConfig(t *testing.T) *config.PayerConfig {
	return &config.PayerConfig{
		...
		API: config.ApiOptions{
			Port:     network.FindFreePort(t),
			HTTPPort: network.FindFreePort(t),
		},
		Contracts: testutils.NewContractsOptions(t, testutils.BLOCKCHAIN_RPC_URL),
