...
type PayerReport struct {
	...
	PayersLeafCount uint32 `protobuf:"varint,5,opt,name=payers_leaf_count,json=payersLeafCount,proto3" json:"payers_leaf_count,omitempty"`
	// The merkle root of the nodes in the report
	NodesMerkleRoot []byte `protobuf:"bytes,6,opt,name=nodes_merkle_root,json=nodesMerkleRoot,proto3" json:"nodes_merkle_root,omitempty"`
	// The number of nodes in the report
	NodesLeafCount uint32 `protobuf:"varint,7,opt,name=nodes_leaf_count,json=nodesLeafCount,proto3" json:"nodes_leaf_count,omitempty"`
...
func (x *PayerReport) GetPayersLeafCount() uint32 {
	if x != nil {
		return x.PayersLeafCount
	}
	return 0
}

func (x *PayerReport) GetNodesMerkleRoot() []byte {
	if x != nil {
		return x.NodesMerkleRoot
	}
	return nil
}

func (x *PayerReport) GetNodesLeafCount() uint32 {
	if x != nil {
		return x.NodesLeafCount
	}
	return 0
}
...
const file_xmtpv4_envelopes_payer_report_proto_rawDesc = "" +
	"\n" +
	"#xmtpv4/envelopes/payer_report.proto\x12\x15xmtp.xmtpv4.envelopes\x1a%identity/associations/signature.proto\"\xbf\x02\n" +
	"\vPayerReport\x12,\n" +
	...
	"\x11payers_leaf_count\x18\x05 \x01(\rR\x0fpayersLeafCount\x12*\n" +
	"\x11nodes_merkle_root\x18\x06 \x01(\fR\x0fnodesMerkleRoot\x12(\n" +
	"\x10nodes_leaf_count\x18\a \x01(\rR\x0enodesLeafCount\"\x8a\x01\n" +
