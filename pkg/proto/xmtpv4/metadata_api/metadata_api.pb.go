...
type GetPayerInfoRequest struct {
	...
	AfterUnixSeconds  uint64                 `protobuf:"varint,3,opt,name=after_unix_seconds,json=afterUnixSeconds,proto3" json:"after_unix_seconds,omitempty"`
	BeforeUnixSeconds uint64                 `protobuf:"varint,4,opt,name=before_unix_seconds,json=beforeUnixSeconds,proto3" json:"before_unix_seconds,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}
...
func (x *GetPayerInfoRequest) GetAfterUnixSeconds() uint64 {
	if x != nil {
		return x.AfterUnixSeconds
	}
	return 0
}

func (x *GetPayerInfoRequest) GetBeforeUnixSeconds() uint64 {
	if x != nil {
		return x.BeforeUnixSeconds
	}
	return 0
}
...
const file_xmtpv4_metadata_api_metadata_api_proto_rawDesc = "" +
	...
	"\aversion\x18\x01 \x01(\tR\aversion\"\xee\x01\n" +
	"\x13GetPayerInfoRequest\x12'\n" +
	"\x0fpayer_addresses\x18\x01 \x03(\tR\x0epayerAddresses\x12P\n" +
	"\vgranularity\x18\x02 \x01(\x0e2..xmtp.xmtpv4.metadata_api.PayerInfoGranularityR\vgranularity\x12,\n" +
	"\x12after_unix_seconds\x18\x03 \x01(\x04R\x10afterUnixSeconds\x12.\n" +
	"\x13before_unix_seconds\x18\x04 \x01(\x04R\x11beforeUnixSeconds\"\x8c\x04\n" +
