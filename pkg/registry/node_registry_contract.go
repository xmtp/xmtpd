...
/*
SmartContractRegistry notifies listeners of changes to the nodes by polling the contract
...
/*
Start loads the initial state from the contract and starts a background refresh loop.
...
func (s *SmartContractRegistry) loadUnfilteredFromContract() ([]Node, error) {
	ctx, cancel := context.WithTimeout(s.ctx, contractCallTimeout)
	defer cancel()
